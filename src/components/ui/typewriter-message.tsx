...
export default function TypewriterMessage({
  ...
  const markdownComponents = {
    ...
    p: ({ children, ...props }: any) => {
      ...
      const hasBlockContent = children && Array.isArray(children) &&
      ...
      // Use div for paragraphs that might contain block elements
      const Component = hasBlockContent ? 'div' : 'p'

      return (
        <Component className="text-sm text-gray-700 dark:text-gray-300 mb-3 leading-relaxed" {...props}>
          {children}
        </Component>
      )
    },
    ...
    pre: ({ ...props }: any) => (
      <pre className="bg-gray-100 dark:bg-gray-800 p-3 rounded text-sm font-mono text-gray-800 dark:text-gray-200 overflow-x-auto mb-3" {...props} />
    )
  }
