...
export function generatePolicyAnalysisPDF(policyData: PolicyData): void {
  ...
  const accentColor = '#e74c3c' // Scotty orange (minimal use)
  ...
  addText('Scotty Adjusting Intelligence Platform', pageWidth/2, 18, {
    fontSize: 10,
    color: '#ffffff',
    align: 'center'
  })
  ...
  checkPageBreak(65)
  yPosition = addSection('CRITICAL ALERTS FOR PUBLIC ADJUSTERS', yPosition)

  // Calculate height needed for the alert box (approximately 8 lines of text plus padding)
  const alertBoxHeight = 55

  pdf.setFillColor(255, 243, 224) // Light warning background
  pdf.rect(margin, yPosition - 2, contentWidth, alertBoxHeight, 'F')

  yPosition = addText('DEPRECIATION WARNING (AP4981):', margin + 2, yPosition + 2, { fontStyle: 'bold', color: accentColor })
  yPosition = addText('Castle Key WILL depreciate labor, overhead and profit in ACV calculations', margin + 2, yPosition)
  ...
  addText('This analysis was generated by Scotty Claims Intelligence System', pageWidth/2, footerY, {
    fontSize: 9,
...
export function generateSimplePolicyPDF(content: string, filename: string): void {
  ...
  pdf.setFontSize(10)
  pdf.text('Scotty Adjusting Intelligence Platform', pageWidth/2, 18, { align: 'center' })
