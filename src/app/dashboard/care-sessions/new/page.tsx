...
const SEVERITY_LEVELS = [
  ...
  { value: 'major', label: 'Major', shortLabel: 'Major', color: 'border-amber-500 bg-amber-50 text-amber-800', icon: '⚠️' },
...
function NewInspectionPageContent() {
  ...
  const handleSubmit = async () => {
    ...
    localStorage.setItem(`inspection-${sessionId}`, JSON.stringify(quickClaimData))
    ...
    localStorage.setItem(`inspection-${sessionId}-data`, JSON.stringify(sessionData))

    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, 1000))

    // Navigate to the inspection start page with the new data
    router.push(`/dashboard/inspection/${sessionId}/start?new=true`)
  }
